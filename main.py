...
def main(page: ft.Page):
    ...
    def copiar(e):
        texto = f"💰 CIERRE - {datetime.now()}\nLocal Ventas: {local_ventas.value}\nTotal Ventas: {total_ventas.value}"
    ...
    page.add(
        ...
        ft.Text("Resultados Locales", weight=ft.FontWeight.BOLD),
        ft.Row([ft.Text("Ventas efectivo:"), local_ventas]),
        ft.Row([ft.Text("Efectivo final:"), local_ideal]),
        ft.Row([ft.Text("Diferencia:"), local_diferencia]),
        ft.Row([ft.Text("Estado final:"), local_estado]),
        ft.Divider(),
        ft.Text("💳 TOTAL", size=18, weight=ft.FontWeight.BOLD),
        pos_ventas, fondo_ini, salidas_total, caja_contada, pedidos_ya, depositos_total,
        ft.ElevatedButton("Reset Total", on_click=reset_total),
        ft.Text("Resultados Totales", weight=ft.FontWeight.BOLD),
        ...
        ft.Row([ft.Text("Estado cuentas:"), total_estado]),
        ft.ElevatedButton("Copiar resultados", on_click=copiar),
    )
    actualizar(None)
